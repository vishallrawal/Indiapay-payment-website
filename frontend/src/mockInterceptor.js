...
// If it fails, we fall back to mock mode even on localhost!
let backendReachable = true;

const checkBackend = async () => {
  if (IS_MOCK_MODE) {
    console.log('[Mock Interceptor] Running in production static environment. Activating Local Mock Database...');
    setupInterceptor();
    return;
  }
  try {
    const res = await originalFetch('/health', { method: 'GET' });
    if (!res.ok) throw new Error();
  } catch (err) {
    console.warn('Real backend server not responding. Activating local mock database fallback...');
